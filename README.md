...
```bash
save "오늘 결정한 것"                # save --category decision "X 선택"
recall "지난주 회의"                 # recall --limit 10 "스트레스"
pattern --week                       # pattern --month --category tasks
recent --days 7
profile show                         # profile update
```

카테고리: `decision` / `learning` / `idea` / `feeling` / `people`
...
| 호스팅 | Railway (가격 정책 배포 전 재확인 필요) | 대안: Fly.io / Render |
...
- **2026-05-18**: 프레임워크 Express → **Fastify**, DB 접근 raw → **Drizzle ORM** 채택. 사유: Fastify 성능/TS 친화, Drizzle은 Supabase·pgvector를 1급으로 지원해 임베딩 검색에 적합.
...
- 설계 원본: Notion "namory — 나모리 프로젝트" (메모리 참조)
